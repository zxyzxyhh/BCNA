...
def sliced_wasserstein_distance(source_z, target_z, embed_dim, num_projections=256, p=1):
    ...
    w_distance = torch.abs(
        torch.sort(proj_target.transpose(0, 1), dim=1)[0] - torch.sort(proj_source.transpose(0, 1), dim=1)[0])
...
def data_load(args):
    ...
    if args.source == ['i']:
        s_dset_path = "/home/b230/data/DA/image_CLEF/list/iList.txt"
    elif args.source == ['p']:
        s_dset_path = "/home/b230/data/DA/image_CLEF/list/pList.txt"
    else:
        s_dset_path = "/home/b230/data/DA/image_CLEF/list/cList.txt"

    if args.target == ['i']:
        t_dset_path = "/home/b230/data/DA/image_CLEF/list/iList.txt"
    elif args.target == ['p']:
        t_dset_path = "/home/b230/data/DA/image_CLEF/list/pList.txt"
    else:
        t_dset_path = "/home/b230/data/DA/image_CLEF/list/cList.txt"
    print(s_dset_path)
    print(t_dset_path)
...
def main(args: argparse.Namespace):
    ...
    F1 = ImageClassifierHead(G.out_features, 12, args.bottleneck_dim, pool_layer).to(device)
    F2 = ImageClassifierHead(G.out_features, 12, args.bottleneck_dim, pool_layer).to(device)
    ...
    mem_cls_1 = torch.ones(len(dset_loaders["target"].dataset), 12).cuda() / 12
    mem_cls_2 = torch.ones(len(dset_loaders["target"].dataset), 12).cuda() / 12
...
def train(dset_loaders, mem_fea, mem_cls_1, mem_cls_2, train_source_iter, train_target_iter,
    ...
    class_weight_src = torch.ones(12, ).to(device)
    ...
    for i in range(args.iters_per_epoch):
        ...
        src_1 = CrossEntropyLabelSmooth(num_classes=12, epsilon=args.smooth)(y1_s, labels_s)
        weight_src_1 = class_weight_src[labels_s].unsqueeze(0)
        loss = torch.sum(weight_src_1 * src_1) / (torch.sum(weight_src_1).item())

        src_2 = CrossEntropyLabelSmooth(num_classes=12, epsilon=args.smooth)(y2_s, labels_s)
        weight_src_2 = class_weight_src[labels_s].unsqueeze(0)
        loss = loss + torch.sum(weight_src_2 * src_2) / (torch.sum(weight_src_2).item())

        loss = loss + (entropy(y1_t) + entropy(y2_t)) * args.trade_off_entropy + Lu * args.mul_weight
        features_target = f_t
        #NA
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--data', metavar='DATA', default='ImageCLEF', choices=utils.get_dataset_names(),
    ...
    parser.add_argument('--self_weight', type=float, default=0.75)
    parser.add_argument('--fixmatch_threshold', default=0.95, type=float)
    parser.add_argument('--mul_weight', type=float, default=0.5)
